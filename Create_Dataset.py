...
def Create_Dataset(data, batch_size, spec_width):
    ...
    for n,sample in enumerate(log_input):
        ...
        for split in range(n_splits):
            new_input.append(sample[:,int((sample.shape[1]-spec_width*(n_splits-split))/2):int((sample.shape[1]-spec_width*(n_splits-2-split))/2)])
            new_label.append(int(label[n].item()))
    weights = [1/new_label.count(i) for i in range(91)]

    X_train, X_val, y_train, y_val = train_test_split(new_input, new_label, test_size=0.2, random_state=8, stratify=new_label)
